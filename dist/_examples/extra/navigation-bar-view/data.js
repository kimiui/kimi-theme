import { jsx as _jsx } from "react/jsx-runtime";
import { Label } from '../../../components/label';
import { SvgColor } from '../../../components/svg-color';
// ----------------------------------------------------------------------
export const NAV_ITEMS = [
    {
        subheader: 'Marketing',
        items: [
            {
                title: 'Landing',
                path: '#landing',
                icon: _jsx(SvgColor, { src: "/assets/icons/navbar/ic-dashboard.svg" }),
                roles: ['admin'],
                caption: 'Display only admin role',
                info: _jsx(Label, { color: "error", children: "+2 " }),
            },
            {
                title: 'Services',
                path: '#services',
                icon: _jsx(SvgColor, { src: "/assets/icons/navbar/ic-analytics.svg" }),
                roles: ['admin', 'user'],
            },
            {
                title: 'Blog',
                path: '#blog',
                icon: _jsx(SvgColor, { src: "/assets/icons/navbar/ic-blog.svg" }),
                info: _jsx(Label, { color: "info", children: "+3 " }),
                children: [
                    {
                        title: 'Item 1',
                        path: '#blog/item-1',
                        caption: 'Display caption',
                        info: '+2',
                    },
                    { title: 'Item 2', path: '#blog/item-2' },
                ],
            },
        ],
    },
    {
        subheader: 'Travel',
        items: [
            {
                title: 'About',
                path: '#about',
                icon: _jsx(SvgColor, { src: "/assets/icons/navbar/ic-user.svg" }),
                info: '+4',
            },
            {
                title: 'Contact',
                path: '#contact',
                icon: _jsx(SvgColor, { src: "/assets/icons/navbar/ic-tour.svg" }),
            },
            {
                title: 'Level',
                path: '#level',
                icon: _jsx(SvgColor, { src: "/assets/icons/navbar/ic-menu-item.svg" }),
                children: [
                    {
                        title: 'Level 2a',
                        path: '#level/2a',
                        icon: _jsx(SvgColor, { src: "/assets/icons/navbar/ic-chat.svg" }),
                        caption: 'This is the caption',
                        children: [
                            { title: 'Level 3a', path: '#level/2a/3a' },
                            {
                                title: 'Level 3b',
                                path: '#level/2a/3b',
                                children: [
                                    { title: 'Level 4a', path: '#level/2a/3b/4a' },
                                    { title: 'Level 4b', path: '#level/2a/3b/4b' },
                                ],
                            },
                            { title: 'Level 3c', path: '#level/2a/3c' },
                        ],
                    },
                    {
                        title: 'Level 2b',
                        path: '#level/2b',
                        icon: _jsx(SvgColor, { src: "/assets/icons/navbar/ic-mail.svg" }),
                    },
                    {
                        title: 'Level 2c',
                        path: '#level/2c',
                        icon: _jsx(SvgColor, { src: "/assets/icons/navbar/ic-calendar.svg" }),
                    },
                ],
            },
        ],
    },
];
