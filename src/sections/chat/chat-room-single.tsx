...
export function ChatRoomSingle({ participant }: Props) {
  const [collapse, collapseActions] = useBoolean(true);
  ...
  return (
    <>
      ...
      <CollapseButton selected={collapse} onClick={collapseActions.toggle}>
        Information
      </CollapseButton>

      <Collapse in={collapse}>{renderContact}</Collapse>
    </>
  );
}
