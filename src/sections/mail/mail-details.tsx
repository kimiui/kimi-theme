...
export function MailDetails({ mail, renderLabel, empty, loading }: Readonly<Props>) {
  const theme = useTheme();

  const [showAttachments, showAttachmentsActions] = useBoolean(true);
  ...
  const renderAttachments = (
    <Stack spacing={1} sx={{ p: 1, borderRadius: 1, bgcolor: 'background.neutral' }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between">
        <ButtonBase
          onClick={showAttachmentsActions.toggle}
          sx={{
            borderRadius: 0.5,
            typography: 'caption',
        ...
        >
          ...
          <Iconify
            icon={showAttachments ? 'eva:arrow-ios-upward-fill' : 'eva:arrow-ios-downward-fill'}
            width={16}
            sx={{ ml: 0.5 }}
          />
        </ButtonBase>
      ...
      <Collapse in={showAttachments} unmountOnExit timeout="auto">
  ...
  const renderContent = (
    <Markdown sx={{ px: 2, '& p': { typography: 'body2' } }}>{mail.message}</Markdown>
  );
