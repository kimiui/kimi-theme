...
export const _ecommerceProductPerformance = [...Array(5)].map((_, index) => {
    const price = _mock.number.price(index);
    const order = _mock.number.nativeM(index);
    return {
        id: _mock.id(index),
        name: _mock.productName(index),
        ...
        order,
        sales: price * order,
        stock: index === 0 ? 0 : _mock.number.nativeL(index),
    };
});
